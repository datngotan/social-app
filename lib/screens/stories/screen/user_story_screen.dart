...
import 'package:socialNetwork/screens/stories/component/highlight_component.dart';
import 'package:socialNetwork/screens/stories/component/my_story_component.dart';
import 'package:socialNetwork/screens/stories/component/new_story_highlight_dialog.dart';
import 'package:socialNetwork/screens/stories/screen/create_story_screen.dart';
      ...
      if (file != null) {
        if (file == FileTypes.CAMERA) {
          ...
          await getImageSource(isCamera: true, isVideo: true).then((value) {
            appStore.setLoading(false);
            CreateStoryScreen(cameraImage: value, isCameraVideo: true)
                .launch(context)
                .then((value) {
              if (value ?? false) {
                list.clear();
            ...
            child: PopupMenuButton(
              ...
              itemBuilder: (context) => <PopupMenuEntry>[
                ...
                if (currentValue != 2 && appStore.showStoryHighlight == 1)
                  PopupMenuItem(
                    value: 2,
                    child: Text(language.highlightStories,
                        style: primaryTextStyle()),
                  ),
                if (currentValue != 3 &&
                    storyActions.validate().any((element) =>
                        element.action == StoryHighlightOptions.trash))
                  PopupMenuItem(
                    value: 3,
                    child:
                        Text(language.draftStories, style: primaryTextStyle()),
                  ),
                if (currentValue != 4 &&
                    storyActions.validate().any((element) =>
                        element.action == StoryHighlightOptions.trash))
                  PopupMenuItem(
                    value: 4,
                    child:
                        Text(language.trashStories, style: primaryTextStyle()),
                  ),
              ],
            ),
          ),
