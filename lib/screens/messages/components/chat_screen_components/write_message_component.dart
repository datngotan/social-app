...
import 'package:socialNetwork/models/messages/mentions_model.dart';
import 'package:socialNetwork/screens/messages/functions.dart';
import 'package:socialNetwork/screens/messages/screens/message_media_screen.dart';
...
enum WriteMessageCallbacks {
  ClearReplyMessage,
  GetThread,
  SendingMessage,
  SendingMessageRejected
}
...
class _WriteMessageComponentState extends State<WriteMessageComponent> {
  GlobalKey<FlutterMentionsState> messageContentKey =
  ...
  void initState() {
    ...
    LiveStream().on(SendMessage, (p0) {
      ...
      sendMessage(
        ...
        messageId: widget.replyMessage != null
            ? widget.replyMessage!.messageId.validate()
            : null,
      ).then((value) {
        tempText = null;
      }).catchError((error) {
    ...
    if (appendedText.contains('@')) {
      widget.users!.forEach((element) {
        ...
        if (appendedText.contains('@${temp.display}')) {
          appendedText = appendedText.validate().replaceAll('@${temp.display}',
              '<span class="bm-mention" data-user-id="${temp.id}">@${temp.display}</span>');
        }
      });
    }
                ...
                children: [
                  IconButton(
                    onPressed: () {
                      ...
                      setState(() {});
                      widget.callback
                          ?.call(WriteMessageCallbacks.ClearReplyMessage);
                    },
                  ...
                  SizedBox(
                    ...
                    child: Column(
                      ...
                      children: [
                        Text(
                            widget.isEditMessage.validate()
                                ? language.editMessage
                                : language.replyToMessage,
                            style: boldTextStyle(size: 14),
                            overflow: TextOverflow.ellipsis,
                            maxLines: 1),
                        Text(widget.replyMessage!.message.validate(),
          ...
          Row(
            children: [
              ...
              FlutterMentions(
                ...
                onChanged: (text) {
                  String message =
                      '42["${SocketEvents.writing}",${widget.threadId},""]';
                  log('$message');
                ...
                onPressed: () {
                  ifNotTester(() async {
                    if (messageContentKey
                      ...
                      widget.callback
                          ?.call(WriteMessageCallbacks.SendingMessage);
                      showReply = false;
                      setState(() {});

                      if (widget.isEditMessage.validate() &&
                          widget.replyMessage != null) {
                        editMessage(
                          ...
                          if (appStore.isWebsocketEnable != 1) {
                            Messages message = Messages(
                              threadId: widget.threadId,
                              dateSent: DateFormat(DATE_FORMAT_1)
                        ...
                        if (widget.replyMessage != null) {
                          sendMessage(
                            ...
                            widget.callback?.call(
                                WriteMessageCallbacks.SendingMessageRejected);
                          });
                        } else {
                          ...
                          if (appStore.isWebsocketEnable == 1) {
                            ...
                            if (await isNetworkAvailable())
                              messageContentKey.currentState!.controller!
                                  .clear();
                            String tempId =
                                generateTempId(threadId: widget.threadId);
