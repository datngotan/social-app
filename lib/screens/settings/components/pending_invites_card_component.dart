...
import 'package:socialNetwork/screens/settings/screens/pending_invites_screen.dart';
...
class PendingInvitesCardComponent extends StatefulWidget {
  ...
  @override
  State<PendingInvitesCardComponent> createState() =>
      _PendingInvitesCardComponentState();
}

class _PendingInvitesCardComponentState
    extends State<PendingInvitesCardComponent> {
  late String formattedDate;

  @override
  void initState() {
    super.initState();
    formattedDate = DateFormat(DATE_FORMAT_3).format(DateTime.parse(
        widget.inviteList.dateModified.validate().substring(0, 10)));
  }
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
            color: selectedInviteRequests.contains(widget.inviteList.id)
                ? greenColor.withAlpha(20)
                : context.cardColor,
                    ...
                    .onTap(() {
                      if (selectedInviteRequests.isEmpty)
                        widget.callback?.call(
                            widget.inviteList.id.validate().toInt(), true);
                    })
                    ...
                    .onTap(() {
                      if (selectedInviteRequests.isEmpty)
                        showConfirmDialogCustom(
                          ...
                          onAccept: (s) {
                            widget.callback?.call(
                                widget.inviteList.id.validate().toInt(), false);
                          },
