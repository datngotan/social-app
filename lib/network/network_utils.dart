...
import 'package:socialNetwork/configs.dart';
import 'package:socialNetwork/main.dart';
import 'package:socialNetwork/network/rest_apis.dart';
import 'package:socialNetwork/utils/common.dart';
import 'package:socialNetwork/utils/constants.dart';
import 'package:socialNetwork/utils/woo_commerce/query_string.dart';
...
Future<Response> buildHttpResponse(
  ...
  if (await isNetworkAvailable()) {
    ...
    if (appStore.isLoggedIn &&
      ...
      if (!res.code.validate().contains('wocommerce_rest') &&
          !res.code.validate().contains('woocommerce_rest')) {
...
Future handleResponse(Response response,
    {bool? isFriendList, bool? avoidTokenError}) async {
  ...
  if (response.statusCode.isSuccessful()) {
    if (jsonDecode(response.body.trim()).runtimeType != (List<dynamic>) &&
        jsonDecode(response.body.trim()).runtimeType != bool) {
...
Future<void> reGenerateToken() async {
  ...
  return await loginUser(
          request: request, isSocialLogin: appStore.isSocialLogin)
      .then((value) async {
    //
  }).catchError((e) {
    throw e;
