...
import 'package:socialNetwork/components/loading_widget.dart';
import 'package:socialNetwork/main.dart';
import 'package:socialNetwork/models/messages/fast_message_model.dart';
...
import 'package:socialNetwork/models/messages/permissions.dart';
import 'package:socialNetwork/models/messages/stream_message.dart';
import 'package:socialNetwork/models/messages/threads_model.dart';
import 'package:socialNetwork/network/messages_repository.dart';
import 'package:socialNetwork/screens/messages/components/chat_screen_components/chat_screen_popup_menu_component.dart';
import 'package:socialNetwork/screens/messages/components/no_message_component.dart';
import 'package:socialNetwork/screens/messages/components/reaction_bottom_sheet.dart';
import 'package:socialNetwork/screens/messages/components/recent_message_component.dart';
import 'package:socialNetwork/screens/messages/functions.dart';
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void initState() {
    ...
    LiveStream().on(ThreadMessageReceived, (p0) {
      int messageId =
          LiveStream().getValue(ThreadMessageReceived).toString().toInt();

      addMessage(messageId: messageId);
    });

    LiveStream().on(FastMessage, (p0) {
      FastMessageModel message = FastMessageModel.fromJson(
          jsonDecode(LiveStream().getValue(FastMessage).toString()));

      addMessage(fastMessage: message);
    });

    LiveStream().on(AbortFastMessage, (p0) {
      AbortMessageModel message = AbortMessageModel.fromJson(
          jsonDecode(LiveStream().getValue(AbortFastMessage).toString()));
    ...
    LiveStream().on(MetaChanged, (p0) {
      StreamMessage object =
          LiveStream().getValue(MetaChanged) as StreamMessage;
  ...
  Future<void> getThread({bool showLoader = true}) async {
    ...
    await getSpecificThread(threadId: widget.threadId).then((value) async {
      ...
      if (thread!.participantsCount.validate() >= 2) {
        widget.user = value.users!.firstWhere(
            (element) => element.userId == thread!.participants!.first);
      } else {
      ...
      getMentionsMembers();
      addMessageIds();
      if (thread!.permissions!.canReplyMsg.toString() != '[]' &&
          thread!.permissions!.canReplyMsg != null) {
        canReplyMsg = CanReplyMsg.fromJson(thread!.permissions!.canReplyMsg);
      } else {
        canReplyMsg = null;
      }

      if (thread!.chatBackground != null &&
          thread!.chatBackground!.url.validate().isNotEmpty) {
  ...
  Future<void> addMessage(
      {int? messageId, FastMessageModel? fastMessage}) async {
    if (fastMessage != null) {
      ...
      await getMessage(messageId: messageId.validate()).then((value) async {
        if (messages.any((element) => element.messageId == messageId)) {
          messages[messages
              .indexWhere((element) => element.messageId == messageId)] = value;
          sendingMessage = false;

          setState(() {});
        } else if (messages.any((element) => element.tmpId == value.tmpId) &&
            value.tmpId.validate().isNotEmpty) {
          messages[messages
              .indexWhere((element) => element.messageId == messageId)] = value;
          sendingMessage = false;
  ...
  Future<void> addReactionList() async {
    for (int i = 0; i <= emojiList.length - 1; i++) {
      reactionList.add(Reaction(
          icon: Text(emojiList[i].skins.validate().first.native.validate(),
              style: TextStyle(fontSize: 20)),
          emojiId: emojiList[i].skins.validate().first.unified.validate()));
    }
    setState(() {});
  }
  ...
  Future<void> loadMore() async {
    ...
    await loadMoreMessages(
            threadId: thread!.threadId.validate(), messageIds: messageIds)
        .then((value) {
        ...
        appBar: AppBar(
          ...
          actions: [
            if (selectedMessage != null)
              Row(
                children: [
                  if (thread!.permissions!.canReply.validate() &&
                      selectedMessage!.senderId.toString() !=
                    ...
                    InkWell(
                      ...
                      child:
                          Icon(Icons.reply, size: 20, color: context.iconColor),
                      ...
                      onTap: () {
                        if (selectedMessage!.favorited == 0) {
                          ...
                          starMessage(
                              ...
                              messageId: selectedMessage!.messageId.validate(),
                              type: FavouriteType.star);
                        } else {
                          selectedMessage!.favorited = 0;
                          starMessage(
                              threadId: widget.threadId,
                              messageId: selectedMessage!.messageId.validate(),
                              type: FavouriteType.unStar);
                        }
                        selectedMessage = null;
                        setState(() {});
                      },
                  ...
                  InkWell(
                    onTap: () {
                      ...
                      Clipboard.setData(new ClipboardData(
                          text: selectedMessage!.message.validate()));
                      selectedMessage = null;
                      setState(() {});
                    },
                    splashColor: Colors.transparent,
                  ...
                  if (thread!.permissions!.canEditOwnMessages.validate() &&
                      selectedMessage!.senderId.toString() ==
                          appStore.loginUserId)
                    InkWell(
                      onTap: () {
                      ...
                      highlightColor: Colors.transparent,
                      child:
                          Icon(Icons.edit, size: 20, color: context.iconColor),
                    ).paddingSymmetric(horizontal: 6),
                  if (thread!.permissions!.canDeleteAllMessages.validate() ||
                      (thread!.permissions!.canDeleteOwnMessages.validate() &&
                          selectedMessage!.senderId.toString() ==
                              appStore.loginUserId))
                    InkWell(
                      onTap: () {
                        messages.removeWhere(
                            (element) => element == selectedMessage);

                        if (appStore.isWebsocketEnable != 1) {
                          messages.removeWhere((element) =>
                              element.messageId ==
                              selectedMessage!.messageId.validate());
                          setState(() {});
                        }
          ...
          title: ChatScreenTitleComponent(
              users: users, thread: thread, user: widget.user),
          leading: IconButton(
            ...
            children: [
              ...
              if (messages.isNotEmpty)
                SizedBox(
                  ...
                  child: ListView.builder(
                    ...
                    padding: EdgeInsets.only(
                        top: 16,
                        bottom: selectedMessage != null || isEditMessage
                            ? 120
                            : 80),
                    ...
                    itemBuilder: (context, index) {
                      ...
                      MessageMeta? meta;
                      bool isLoggedInUser =
                          message.senderId.toString() == appStore.loginUserId;
                      Messages? repliedMessage;
                      MessagesUsers? repliedUser;
                      MessagesUsers? sender;

                      sender =
                          users.firstWhere((e) => e.userId == message.senderId);
                      ...
                      String lastDate = index != 0
                          ...
                          : "";
                      String date =
                          getChatDate(date: message.dateSent.validate());
                      ...
                      if (message.meta.toString() != '[]' &&
                        ...
                        if (meta.reactions != null &&
                            meta.reactions.validate().isNotEmpty) {
                          if (thread!.type == 'group' ||
                              thread!.participantsCount.validate() > 2) {
                            meta.reactions.validate().forEach((element) {
                              if (element.users
                                  .validate()
                                  .contains(appStore.loginUserId.toInt())) {
                                int index = meta!.reactions
                                    .validate()
                                    .indexWhere((e) => e.users
                                        .validate()
                                        .contains(
                                            appStore.loginUserId.toInt()));

                                reactionList.validate().forEach(
                                  (element) {
                                    if (element.emojiId.validate() ==
                                        meta!.reactions
                                            .validate()[index]
                                            .reaction
                                            .validate()) {
                                      int iconIndex = emojiList.indexWhere(
                                          (e) =>
                                              e.skins
                                                  ...
                                                  .unified
                                                  .validate() ==
                                              element.emojiId.validate());
                                      reactionIcon = emojiList[iconIndex]
                                          .skins
                                          .validate()
                                          .first
                                          .native
                              ...
                              (element) {
                                if (element.emojiId.validate() ==
                                    meta!.reactions
                                        ...
                                        .reaction
                                        .validate()) {
                                  int iconIndex = emojiList.indexWhere((e) =>
                                      e.skins
                                          .validate()
                                          .first
                                          .unified
                        ...
                        if (meta.replyTo != null) {
                          messages.forEach((element) {
                            if (element.messageId == meta!.replyTo.validate()) {
                              ...
                              repliedUser = users.firstWhere(
                                  (e) => e.userId == element.senderId);
                            }
                          });
                        }
                      }
                      ...
                      if (thread != null)
                        return Column(
                          children: [
                            if (index == 0 && showPaginationLoader)
                              ThreeBounceLoadingWidget(),
                            ChatScreenMessagesComponent(
                              participantCount:
                                  thread!.participantsCount.validate(),
                              ...
                              message: message,
                              meta: meta,
                              time: message.lastUpdate.validate() == 0
                                  ? timeOfMessage(message.dateSent.validate())
                                  : timeStampToDateMessage(
                                      message.lastUpdate.validate()),
                              threadType: thread!.type,
                              reactionList: reactionList,
                              emojiId: emoji,
                              reactionIcon: reactionIcon,
                              callback: (value, String? emoji, int? messageId,
                                  bool? isReactionRemoved) async {
                                switch (value) {
                                  case ChatScreenMessagesComponentCallbacks
                                        .onTap:
                                    if (selectedMessage != null) {
                                      selectedMessage = null;
                                      setState(() {});
                                  ...
                                  case ChatScreenMessagesComponentCallbacks
                                    ...
                                    await saveReaction(
                                            messageId: messageId.validate(),
                                            emoji: emoji.validate())
                                        .then((value) async {
                                      value.messages
                                          .validate()
                                          .forEach((element) {
                            ...
                            if (index == messages.length - 1 && sendingMessage)
                              ThreeBounceLoadingWidget()
                          ],
                        );
                      else
                ...
                SizedBox(
                    width: context.width(),
                    child:
                        NoMessageComponent().paddingSymmetric(horizontal: 16))
              else if (isError && !appStore.isLoading)
                SizedBox(
                    width: context.width(),
                    child: NoMessageComponent(errorText: errorText)
                        .paddingSymmetric(horizontal: 16)),
              if (canReplyMsg != null &&
                  canReplyMsg!.userBlockedMessages != null)
                Align(
                  ...
                  child: Container(
                    ...
                    child: Text(
                      canReplyMsg!.userBlockedMessages.validate().isNotEmpty
                          ? canReplyMsg!.userBlockedMessages.validate()
                          : '',
                ...
                Positioned(
                  ...
                  child: WriteMessageComponent(
                    threadSecretKey:
                        thread != null ? thread!.secretKey.validate() : "",
                    ...
                    onAddMessage: (value) {
                      if (messages.any(
                          (element) => element.messageId == value.messageId)) {
                        messages[messages.indexWhere((element) =>
                            element.messageId == value.messageId)] = value;
                        sendingMessage = false;

                        setState(() {});
                      } else if (messages
                          .any((element) => element.tmpId == value.tmpId)) {
                        messages[messages.indexWhere(
                            (element) => element.tmpId == value.tmpId)] = value;
                        sendingMessage = false;
