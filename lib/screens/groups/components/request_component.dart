...
class _RequestComponentState extends State<RequestComponent>
  ...
  Future<List<GroupRequestModel>> getRequests() async {
    await getGroupMembershipRequest(
            groupId: widget.groupId.validate(), page: mPage)
        .then((value) {
                                  ...
                                  onTap: () async {
                                    if (!appStore.isLoading)
                                      ifNotTester(() async {
                                        ...
                                        await acceptGroupMembershipRequest(
                                                requestId:
                                                    member.requestId.validate())
                                            .then((value) {
                                          mPage = 1;
                                          appStore.setLoading(true);
                                          future = getRequests();
                                          LiveStream()
                                              .emit(OnGroupRequestAccept);
                                          widget.onRequestAccept?.call(
                                              member.requestId.validate());
                                        }).catchError((e) {
                                          appStore.setLoading(false);
                                  ...
                                  onPressed: () async {
                                    if (!appStore.isLoading)
                                      ifNotTester(() async {
                                        ...
                                        await rejectGroupMembershipRequest(
                                                requestId:
                                                    member.requestId.validate())
                                            .then((value) {
                                          mPage = 1;
