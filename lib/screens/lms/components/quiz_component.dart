...
import 'package:socialNetwork/screens/lms/components/fill_blanks_component.dart';
import 'package:socialNetwork/screens/lms/components/single_choice_component.dart';
...
class _QuizComponentState extends State<QuizComponent> {
    ...
    if (!widget.isReviewQuiz) {
      lmsStore.quizList.forEach((element) {
        if (element.quizId == widget.quiz.id) {
          startSeconds = DateTime.now()
              .difference(DateTime.parse(element.startQuizTime))
              .inSeconds;
        }

        if (widget.quiz.duration!.split(' ').last == 'minutes') {
          if (widget.quiz.duration!.split(' ').first.toInt() * 60 >
              startSeconds) {
            timeLeft = widget.quiz.duration!.split(' ').first.toInt() * 60 -
                startSeconds;
          } else {
            finishQuizFunc();
          }
          ...
          if (widget.quiz.duration!.split(' ').first.toInt() * 60 >
              startSeconds) {
            showInHours = true;
            timeLeft = (widget.quiz.duration!.split(' ').first.toInt() * 3600) -
                startSeconds;
          } else {
            finishQuizFunc();
          }
  ...
  startTimer() {
    timer = Timer.periodic(Duration(seconds: 1), (time) {
      ...
      if (showInHours) {
        timeToShow = hours.toString().padLeft(2, "0") +
            ':' +
    ...
    ifNotTester(() {
      if (lmsStore.quizList.isNotEmpty) {
        ...
        lmsStore.quizList.first.answers.validate().forEach((element) {
          ans.putIfAbsent(
              element.questionId..validate(), () => element.value.validate());
        });

        Map request = {
          "id": lmsStore.quizList
              .firstWhere((element) => element.quizId == widget.quiz.id)
              .quizId,
          "answered": ans,
        };
        ...
        finishQuiz(request: request).then((value) async {
          lmsStore.quizList
              .removeWhere((element) => element.quizId == widget.quiz.id);
          await setValue(
              SharePreferencesKey.LMS_QUIZ_LIST, jsonEncode(lmsStore.quizList));
                ...
                Container(
                  decoration: BoxDecoration(
                    ...
                    borderRadius: radiusOnly(
                        topRight: commonRadius, topLeft: commonRadius),
                  ),
                  ...
                  child: Row(
                    ...
                    children: [
                      Text(
                          '${language.questions}:  $pageIndex/${widget.quiz.results!.results!.question_count.validate()}',
                          style: primaryTextStyle(color: Colors.white)),
                      if (!widget.isReviewQuiz)
                        TextIcon(
                          text: timeToShow,
                          textStyle: primaryTextStyle(color: Colors.white),
                          prefix: cachedImage(ic_timer,
                              height: 18,
                              width: 18,
                              fit: BoxFit.cover,
                              color: Colors.white),
                    ...
                    children: [
                      PageView.builder(
                        ...
                        itemBuilder: (BuildContext context, int index) {
                          Question question =
                              widget.quiz.questions.validate()[index];

                          return Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text('${index + 1}. ' + question.title.validate(),
                              ...
                              if (question.type == QuestionType.single_choice ||
                                ...
                                SingleChoiceComponent(
                                  ...
                                  answered: widget.answers.validate().isNotEmpty
                                      ? widget.answers.validate()[index]
                                      : null,
                              ...
                              if (widget.isReviewQuiz &&
                                  widget.answers.validate().isNotEmpty)
                                Container(
                                  decoration: BoxDecoration(
                                      color: widget.answers
                                              .validate()[index]
                                              .correct
                                              .validate()
                                          ? appGreenColor
                          ...
                          children: [
                            ...
                            Container(
                              ...
                              child: ListView.builder(
                                ...
                                itemCount:
                                    widget.quiz.questions.validate().length,
