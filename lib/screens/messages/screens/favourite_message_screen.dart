...
import 'package:socialNetwork/screens/messages/components/message_media_component.dart';
                ...
                if (snap.messages.validate().isNotEmpty) {
                  return ListView.separated(
                    ...
                    itemBuilder: (ctx, index) {
                      ...
                      Threads thread = snap.threads.validate().firstWhere(
                          (element) => element.threadId == message.threadId);
                      MessagesUsers user = snap.users.validate().firstWhere(
                          (element) =>
                              element.userId == thread.participants!.first);

                      DateTime dateTime =
                          DateTime.parse(message.dateSent.validate());
                            ...
                            SizedBox(
                              ...
                              child: Column(
                                ...
                                children: [
                                          ...
                                          RichText(
                                            text: TextSpan(
                                              children: [
                                                TextSpan(
                                                    text:
                                                        '${thread.type == ThreadType.group ? thread.title.validate() : user.name.validate()} ',
                                                    style: boldTextStyle(
                                                        fontFamily:
                                                            fontFamily)),
                                                if (thread.type ==
                                  ...
                                  16.height,
                                  if (meta != null &&
                                      meta.files.validate().isNotEmpty)
                                    MessageMediaComponent(
                                            files: meta.files.validate())
