...
import 'package:socialNetwork/screens/messages/screens/preview_screen.dart';
...
class SetChatWallpaperScreen extends StatefulWidget {
  ...
  SetChatWallpaperScreen(
      {required this.isGeneralSetting,
      this.sendFile,
      this.threadId,
      this.wallpaperUrl});
...
class _SetChatWallpaperScreenState extends State<SetChatWallpaperScreen> {
  ...
  Future<void> setWallpaper({required File file}) async {
    ...
    saveChatBackground(
      id: widget.isGeneralSetting
          ? appStore.loginUserId
          : widget.threadId.validate().toString(),
      image: file,
  ...
  bool showDeleteButton() {
    if (wallpaperFile != null ||
        (widget.wallpaperUrl != null &&
            widget.wallpaperUrl.validate().isNotEmpty)) {
      if (widget.wallpaperUrl.validate() == messageStore.globalChatBackground &&
          !widget.isGeneralSetting) {
        return false;
      } else {
        return true;
      }
                ...
                Column(
                  ...
                  children: [
                    Container(
                      ...
                      child: Row(
                        children: [
                          cachedImage(AppImages.defaultAvatarUrl,
          ...
          TextButton(
            onPressed: () async {
              ...
              await getImageSource(isCamera: false).then((value) {
                if (value != null) {
                  PreviewScreen(wallPaperFile: value)
                      .launch(context)
                      .then((isChanged) {
                    if (isChanged) {
