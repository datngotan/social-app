...
class _MembersComponentState extends State<MembersComponent> {
  ...
  void popUpMenuButtonActions({required String role, required String action}) {
    return ifNotTester(() {
      ...
      groupMemberRoles(
              ...
              memberId: widget.member.userId.validate(),
              role: role,
              action: action)
          .then((value) {
        if (action == GroupActions.demote &&
            widget.member.userId.toString() == appStore.loginUserId) {
          finish(context);
          finish(context, true);
        }
  ...
  Widget build(BuildContext context) {
    return Row(
              ...
              children: [
                SizedBox(
                  ...
                  child: Row(
                    ...
                    children: [
                      Text(widget.member.userName.validate(),
                              style: boldTextStyle(),
                              overflow: TextOverflow.ellipsis)
                          .flexible(),
                      if (widget.member.isUserVarified.validate())
                      ...
                      if (widget.member.isAdmin.validate())
                        Container(
                          ...
                          child: Text(language.admin,
                              style: secondaryTextStyle(
                                  color: appGreenColor, size: 10)),
                        ),
                      if (widget.member.isMod == 1)
                ...
                6.height,
                Text(widget.member.mentionName.validate(),
                    style: secondaryTextStyle()),
              ],
            ...
            child: PopupMenuButton(
              ...
              onSelected: (val) {
                if (val == 2) {
                  ...
                  popUpMenuButtonActions(
                      action: GroupActions.promote, role: Roles.mod);
                } else if (val == 4) {
                  popUpMenuButtonActions(
                      action: widget.member.isBanned == 1
                          ? GroupActions.unban
                          : GroupActions.ban,
                      role: Roles.member);
                } else if (val == 5) {
                  popUpMenuButtonActions(
                      action: GroupActions.demote, role: Roles.member);
                } else {
                  popUpMenuButtonActions(
                      action: GroupActions.promote, role: Roles.admin);
                }
              },
              ...
              itemBuilder: (context) => <PopupMenuEntry>[
                ...
                if (isMember)
                  PopupMenuItem(
                    value: 4,
                    child: Text(widget.member.isBanned != 1
                        ? language.ban
                        : language.unban),
                    textStyle: primaryTextStyle(),
                  ),
                if (widget.member.isAdmin.validate() ||
                    widget.member.isMod == 1)
                  PopupMenuItem(
                    value: 5,
      ...
      MemberProfileScreen(memberId: widget.member.userId.validate())
          .launch(context);
