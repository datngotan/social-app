    ...
    if (isForeGround) {
      ...
      try {
        ...
        Map<String, dynamic> additionalData =
            jsonDecode(message.data[FirebaseMsgConst.additionalDataKey]) ?? {};

        if (additionalData.isNotEmpty) {
          additionalData.entries.forEach((element) {
            log('-------------------->>${element.key}');
            if (element.key == FirebaseMsgConst.isCommentKey) {
              int postId = additionalData.entries
                  .firstWhere(
                      (element) => element.key == FirebaseMsgConst.postIdKey)
                  .value;
              if (postId != 0) {
