...
import 'package:socialNetwork/models/calls/user_model.dart';
import 'package:socialNetwork/screens/messages/screens/message_screen.dart';
import 'package:socialNetwork/screens/messages/screens/set_chat_wallpaper_screen.dart';
import 'package:socialNetwork/utils/app_constants.dart';
import 'package:socialNetwork/utils/cached_network_image.dart';
import 'package:socialNetwork/utils/call/call_functions.dart';
...
enum ChatScreenPopUpMenuComponentCallbacks {
  MuteOrUnMute,
  GetThread,
  SetState,
  DeleteThread
...
class ChatScreenPopUpMenuComponent extends StatefulWidget {
  ...
  @override
  State<ChatScreenPopUpMenuComponent> createState() =>
      _ChatScreenPopUpMenuComponentState();
}

class _ChatScreenPopUpMenuComponentState
    extends State<ChatScreenPopUpMenuComponent> {
  Future<void> onStartCall({required String callType}) async {
    ...
    Map request = {
      "user_id": widget.user!.userId.validate(),
  ...
  Widget build(BuildContext context) {
    if (widget.thread != null) {
      return Row(
        children: [
          if (widget.thread!.permissions!.canAudioCall.validate() &&
              widget.thread!.participantsCount.validate() <= 2)
            InkWell(
              onTap: () {
              ...
              child: cachedImage(ic_call,
                  height: 20, width: 20, color: context.iconColor),
            ).paddingSymmetric(horizontal: 6),
          if (widget.thread!.permissions!.canVideoCall.validate() &&
              widget.thread!.participantsCount.validate() <= 2)
            InkWell(
              onTap: () {
              ...
              splashColor: Colors.transparent,
              highlightColor: Colors.transparent,
              child: cachedImage(ic_video_call,
                  height: 20, width: 20, color: context.iconColor),
            ).paddingSymmetric(horizontal: 6),
          Theme(
            ...
            child: PopupMenuButton(
              ...
              onSelected: (val) async {
                if (val == 1) {
                  ParticipantsScreen(
                    ...
                    participantsCount:
                        widget.thread!.participantsCount.validate(),
                    threadId: widget.threadId.validate(),
                  ...
                  if (widget.user!.blocked == 0) {
                    ...
                    blockUser(userId: widget.user!.userId.validate())
                        .then((value) {
                      widget.callback?.call(
                          ChatScreenPopUpMenuComponentCallbacks.GetThread);
                    });
                  } else {
                    widget.user!.blocked = 0;
                    appStore.setLoading(true);
                    unblockUser(userId: widget.user!.userId.validate())
                        .then((value) {
                  ...
                  widget.callback
                      ?.call(ChatScreenPopUpMenuComponentCallbacks.SetState);
                } else if (val == 3) {
                  if (widget.thread!.permissions!.isMuted.validate()) {
                    appStore.setLoading(true);
                    await unMuteThread(threadId: widget.threadId.validate())
                        .then((value) {
                      appStore.setLoading(false);
                      widget.callback?.call(
                          ChatScreenPopUpMenuComponentCallbacks.MuteOrUnMute);
                    }).catchError(onError);
                  } else {
                    await muteThread(threadId: widget.threadId.validate())
                        .then((value) {
                      widget.callback?.call(
                          ChatScreenPopUpMenuComponentCallbacks.MuteOrUnMute);
                    }).catchError(onError);
                  }
                  widget.thread!.permissions!.isMuted =
                      !widget.thread!.permissions!.isMuted.validate();
                  widget.callback
                      ?.call(ChatScreenPopUpMenuComponentCallbacks.SetState);
                } else if (val == 4) {
                      ...
                      return FractionallySizedBox(
                        ...
                        child: AddNewParticipantComponent(
                          ...
                          callback: () {
                            finish(context);
                            widget.callback?.call(
                                ChatScreenPopUpMenuComponentCallbacks
                                    .GetThread);
                          },
                        ),
                      );
                  ...
                  showConfirmDialogCustom(
                    ...
                    onAccept: (c) {
                      ifNotTester(() async {
                        await deleteThread(
                                threadId: widget.threadId.validate().validate())
                            .then((value) {
                          widget.callback?.call(
                              ChatScreenPopUpMenuComponentCallbacks
                                  .DeleteThread);
              ...
              itemBuilder: (context) => <PopupMenuEntry>[
                ...
                if (messageStore.allowBlock &&
                    widget.thread!.type == ThreadType.thread &&
                    widget.thread!.participantsCount.validate() == 2)
                  PopupMenuItem(
                    value: 2,
                    child: TextIcon(
                      text: widget.user!.blocked.validate() == 0
                          ? language.block
                          : language.unblock,
                      textStyle: secondaryTextStyle(),
                    ),
                ...
                if (widget.thread!.permissions!.canMuteThread.validate())
                  PopupMenuItem(
                    value: 3,
                    child: TextIcon(
                      text: widget.thread!.permissions!.isMuted.validate()
                ...
                if (widget.thread!.participantsCount.validate() == 2 ||
                    widget.thread!.permissions!.isModerator.validate() ||
                    widget.thread!.meta!.allowInvite.validate())
                  PopupMenuItem(
                    value: 4,
                    child: TextIcon(
                ...
                if (widget.thread!.participantsCount.validate() > 2 &&
                    widget.thread!.type != ThreadType.group)
                  PopupMenuItem(
                    value: 5,
                    child: TextIcon(
                      text: language.viewParticipants,
