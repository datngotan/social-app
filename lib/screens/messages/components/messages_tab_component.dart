...
import 'package:socialNetwork/screens/messages/components/initial_message_component.dart';
import 'package:socialNetwork/screens/messages/components/recent_message_component.dart';
import 'package:socialNetwork/screens/messages/components/restore_component.dart';
...
class _MessagesTabComponentState extends State<MessagesTabComponent> {
  ...
  void initState() {
    ...
    LiveStream().on(RefreshRecentMessage, (p0) {
      ...
      if (threadsList != null) {
        if (threadsList!.threads
            .validate()
            .any((element) => element.threadId == thread.threadId)) {
          addMessage(
              messageId: thread.lastMessage.validate(),
        ...
        if (threadsList != null)
          if (threadsList!.threads.validate().isNotEmpty)
            AnimatedListView(
              ...
              itemBuilder: (context, index) {
                ...
                if (thread.participantsCount.validate() >= 2) {
                  user = threadsList!.users.validate().firstWhere((element) =>
                      element.userId == thread.participants!.first);
                } else {
                  user = MessagesUsers(name: 'Deleted User');
                }
                Messages message = threadsList!.messages.validate().firstWhere(
                    (element) =>
                        element.messageId.validate() ==
                        thread.lastMessage.validate());
                if (thread.participantsCount.validate() > 2 &&
                    thread.type != ThreadType.group) {
                  participantUsers.clear();
                  thread.participants.validate().forEach((val) {
                    if (threadsList!.users
                        .validate()
                        .any((element) => element.userId == val)) {
                      participantUsers.add(threadsList!.users
                          .validate()
                          .firstWhere((element) => element.userId == val));
                    }
                  });
                }
            ...
            InitialMessageComponent(
                title: language.noConversationsYet, showButton: true),
        if (showRestore)
          Positioned(
