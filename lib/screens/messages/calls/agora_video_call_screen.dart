...
import 'package:socialNetwork/models/calls/call_model.dart';
...
class _AgoraVideoCallScreenState extends State<AgoraVideoCallScreen> {
  ...
  Future<void> startMissCallTime() async {
    if (!isUserJoined && missCallSeconds < 30 && mounted) {
      ...
      if (!isUserJoined) {
        ...
        Map request = {
          "thread_id": widget.callModel.threadId,
          "message_id": widget.callModel.messageId,
          "type": widget.callModel.callType,
          "duration": missCallSeconds
        };
  ...
  init() async {
    if (widget.callModel.callType == BetterMessageCallType.video)
      isVideoCall = true;
  ...
  void addPostFrameCallBack() {
    SchedulerBinding.instance.addPostFrameCallback((_) {
      callStreamSubscription = callService
          .callStream(uid: appStore.loginUserId.validate().toString())
          ...
          RawMaterialButton(
            onPressed: () {
              ...
              if (appStore.isWebsocketEnable == 1 &&
                ...
                String message = '';
                if (isUserJoined) {
                  String id = appStore.loginUserId == widget.callModel.callerId
                      ? widget.callModel.receiverId.validate()
                      : widget.callModel.callerId.validate();

                  message =
                      '42["${SocketEvents.mediaEvent}",{"event":"${SocketEvents.callEnd}","to": $id,"thread_id":${widget.callModel.threadId}}]';
                } else if (appStore.loginUserId == widget.callModel.callerId) {
                  message =
                      '42["${SocketEvents.mediaEvent}",{"event":"${SocketEvents.callCancelled}","to":${widget.callModel.receiverId},"thread_id":${widget.callModel.threadId}}]';
                } else {
                  message =
                ...
                children: [
                  cachedImage(
                    widget.isReceiver
                        ...
                        : widget.callModel.receiverPhotoUrl.validate(),
                    height: 120,
                    width: 120,
                    fit: BoxFit.fill,
                  ...
                  Text(
                    widget.isReceiver
                        ...
                        : widget.callModel.receiverName.validate(),
