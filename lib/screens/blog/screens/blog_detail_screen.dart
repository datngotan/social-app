...
import 'package:socialNetwork/models/common_models/wp_embedded_model.dart';
...
import 'package:socialNetwork/screens/blog/components/blog_comment_component.dart';
import 'package:socialNetwork/utils/app_constants.dart';
import 'package:socialNetwork/utils/cached_network_image.dart';
import 'package:socialNetwork/utils/html_widget.dart';
...
class _BlogDetailScreenState extends State<BlogDetailScreen> {
  ...
  @override
  void initState() {
    if (!widget.data.content!.protected.validate() &&
        widget.data.content!.rendered.validate().isNotEmpty)
      future = getComments();
    super.initState();
  ...
  Future<void> getBlog({String? password}) async {
    appStore.setLoading(true);
    await wpPostById(postId: widget.blogId.validate(), password: password)
        .then((value) {
      future = getComments();
      blog = value;
      setState(() {});
        ...
        children: [
          SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                Text(parseHtmlString(blog.title!.rendered.validate()),
                    style: boldTextStyle()),
                16.height,
                  ...
                  RichText(
                    text: TextSpan(
                      children: [
                        TextSpan(
                            text:
                                '${blog.embedded!.author!.first.name.validate()} ',
                            style: secondaryTextStyle(
                                size: 14, fontFamily: fontFamily)),
                        if (blog.embedded!.author!.first.is_user_verified
                            .validate())
                          WidgetSpan(
                        ...
                        TextSpan(
                            text:
                                '  |  ${convertToAgo(blog.date.validate())}  |',
                            style: secondaryTextStyle(
                                size: 14, fontFamily: fontFamily)),
                        TextSpan(
                            text: '  $category',
                ...
                if (blog.content!.protected.validate() &&
                    blog.content!.rendered.validate().isEmpty)
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(language.protectedPostText,
                          style: secondaryTextStyle()),
                      16.height,
                      Row(
                  ...
                  Column(
                    ...
                    children: [
                      if (blog.embedded!.featuredMedia != null)
                        cachedImage(blog
                                .embedded!.featuredMedia!.first.source_url
                                .validate())
                            .cornerRadiusWithClipRRect(commonRadius),
                      16.height,
                      HtmlWidget(
                          postContent: getPostContent(
                              blog.content!.rendered.validate())),
                      Divider(),
                              ...
                              Column(
                                ...
                                children: [
                                  ...
                                  appButton(
                                    ...
                                    context: context,
                                    text:
                                        language.submit.capitalizeFirstLetter(),
                                    onTap: () {
                                      ifNotTester(() {
                                        if (!appStore.isLoading) {
                                          appStore.setLoading(true);
                                          addBlogComment(
                                                  postId: blog.id.validate(),
