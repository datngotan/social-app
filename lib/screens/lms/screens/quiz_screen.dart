...
import 'package:socialNetwork/screens/lms/components/finish_quiz_component.dart';
import 'package:socialNetwork/screens/lms/components/locked_content_widget.dart';
import 'package:socialNetwork/screens/lms/components/quiz_component.dart';
import 'package:socialNetwork/screens/lms/components/start_quiz_component.dart';
  ...
  Future<void> getQuiz() async {
    ...
    await getQuizById(quizId: widget.quizId).then((value) async {
      ...
      if (value.results!.status.validate() == CourseStatus.started) {
        if (!lmsStore.quizList
            .any((element) => element.quizId == widget.quizId)) {
      ...
      if (value.results!.results != null) {
        if (value.results!.results!.graduation.validate() ==
            CourseStatus.passed) {
            ...
            SingleChildScrollView(
              child: Column(
                children: [
                  ...
                  if (quiz.results!.status.validate() == CourseStatus.started ||
                      isReviewQuiz)
                    QuizComponent(
                      quiz: quiz,
                      callback: () {
                  ...
                  if (quiz.results!.status.validate() ==
