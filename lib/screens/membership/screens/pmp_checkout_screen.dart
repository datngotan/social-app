...
import 'package:socialNetwork/screens/membership/components/plan_subtitle_component.dart';
import 'package:socialNetwork/screens/membership/screens/discount_codes_screen.dart';
import 'package:socialNetwork/screens/membership/screens/pmp_order_detail_screen.dart';
import 'package:socialNetwork/services/razor_pay_services.dart';
import 'package:socialNetwork/services/stripe_services.dart';
                  ...
                  ifNotTester(() async {
                    if (payment!.id.validate() == PaymentIds.stripe) {
                      ...
                      RazorPayServices.init(
                        ...
                        razorKey: payment!.mode.validate() == 'testing'
                            ? payment!.testing!.publicKey.validate()
                            : payment!.live!.publicKey.validate(),
                        amount: plan!.isInitial.validate()
                            ? plan!.initialPayment.validate()
                            : plan!.billingAmount.validate(),
                        planId: widget.selectedPlan.id.validate(),
                        ctx: context,
                        paymentMode: payment!.mode.validate(),
                        url: payment!.mode.validate() == 'testing'
                            ? payment!.testing!.url.validate()
                            : payment!.live!.url.validate(),
                        secret: payment!.mode.validate() == 'testing'
                            ? payment!.testing!.key.validate()
                            : payment!.live!.key.validate(),
                      );
                      await 1.seconds.delay;
                      appStore.setLoading(false);
                      RazorPayServices.razorPayCheckout(
                          plan!.isInitial.validate()
                              ? plan!.initialPayment.validate()
                              : plan!.billingAmount.validate());
                    } else {
                      //
