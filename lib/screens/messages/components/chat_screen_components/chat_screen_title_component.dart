...
import 'package:socialNetwork/models/messages/unread_threads.dart';
...
class ChatScreenTitleComponent extends StatefulWidget {
  ...
  ChatScreenTitleComponent(
      {Key? key, this.thread, this.user, required this.users})
      : super(key: key);

  @override
  State<ChatScreenTitleComponent> createState() =>
      _ChatScreenTitleComponentState();
}
                  ...
                  Observer(builder: (_) {
                    if (messageStore.typingList.isNotEmpty) {
                      if (messageStore.typingList.any((element) =>
                          element.threadId == widget.thread!.threadId)) {
                        if (widget.thread!.type == ThreadType.group ||
                            widget.thread!.participantsCount.validate() > 2) {
                          List<UnreadThreadModel> typingList = messageStore
                              .typingList
                              .where((element) =>
                                  element.threadId == widget.thread!.threadId)
                              .toList();
                        ...
                        int similarElementsCount = widget.thread!.participants
                            .validate()
                            .where((element) =>
                                messageStore.onlineUsers.contains(element))
                            .toList()
                            .length;
                        ...
                        return Text(
                          widget.thread!.type == ThreadType.group ||
                              ...
                              : convertToAgo(
                                  widget.user!.lastActive.validate()),
                          style: secondaryTextStyle(size: 12),
                        );
                      } else {
                        return Text(
                      ...
                      if (similarElementsCount != 0) {
                        if (widget.thread!.type == ThreadType.group ||
                          ...
                          return Text(
                            messageStore.onlineUsers.contains(
                                    widget.user!.id.validate().toInt())
                                ? ' ${language.online}'
                                : convertToAgo(
                                    widget.user!.lastActive.validate()),
                            style: secondaryTextStyle(size: 12),
