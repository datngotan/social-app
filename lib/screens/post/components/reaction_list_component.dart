          ...
          children: [
            RichText(
              text: TextSpan(
                children: [
                  TextSpan(
                      text: widget.reaction.user!.displayName.validate() + ' ',
                      style: boldTextStyle(size: 14, fontFamily: fontFamily)),
                  if (widget.reaction.user!.isVerified.validate())
                    WidgetSpan(
            ...
            Text(widget.reaction.user!.userName.validate(),
                style: secondaryTextStyle(size: 12, fontFamily: fontFamily))
          ],
          crossAxisAlignment: CrossAxisAlignment.start,
        ).expand(),
        cachedImage(widget.reaction.icon.validate(), height: 26, width: 26)
            .withTooltip(msg: widget.reaction.reaction.toString())
      ],
    ).onTap(() async {
      MemberProfileScreen(memberId: widget.reaction.user!.id.validate().toInt())
          .launch(context);
    },
        splashColor: Colors.transparent,
        highlightColor: Colors.transparent).paddingSymmetric(vertical: 8);
  }
}
