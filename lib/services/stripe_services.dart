...
import 'package:socialNetwork/models/payment/stripe_pay_model.dart';
import 'package:socialNetwork/models/payment/stripe_payment_details_model.dart';
...
class StripeServices {
  ...
  void stripePay() async {
    ...
    request.bodyFields = {
      ...
      'description':
          'Name: ${appStore.loginFullName} - Email: ${appStore.loginEmail}',
    };
        ...
        if (response.statusCode.isSuccessful()) {
          ...
          SetupPaymentSheetParameters setupPaymentSheetParameters =
            ...
            applePay: PaymentSheetApplePay(
                merchantCountryCode: kReleaseMode
...
Future<void> getStripePaymentDetails({
  ...
  var request =
      http.Request(HttpMethodType.GET.name, Uri.parse('$stripeURL/$id'));
  request.headers.addAll(headers);

  request.send().then((value) async {
    http.Response.fromStream(value).then((response) async {
      if (response.statusCode.isSuccessful()) {
        StripePaymentDetailsModel res =
            StripePaymentDetailsModel.fromJson(await handleResponse(response));
        ...
        Map request = {
          ...
          "card_details": {
            "card_name":
                "${res.charges!.data.validate().first.paymentMethodDetails!.card!.brand!.validate()}",
            "account_number":
                "${res.charges!.data.validate().first.paymentMethodDetails!.card!.last4!.validate()}",
            "exp_month": res.charges!.data
                .validate()
                .first
          ...
          "meta_value": "",
          "transaction_id":
              res.charges!.data.validate().first.balanceTransaction.validate(),
          "level_id": levelId,
