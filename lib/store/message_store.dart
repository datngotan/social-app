...
abstract class MessageStoreBase with Store {
  ...
  Future<void> setUserNameKey(String val, {bool isInitializing = false}) async {
    userNameKey = val;
    if (!isInitializing)
      await setValue(SharePreferencesKey.USERNAME_KEY, '$val');
  }

  @action
  Future<void> setUserAvatarKey(String val,
      {bool isInitializing = false}) async {
    userAvatarKey = val;
    if (!isInitializing)
      await setValue(SharePreferencesKey.USER_AVATAR_KEY, '$val');
  }
  ...
  Future<void> setBmSecretKey(String val, {bool isInitializing = false}) async {
    bmSecretKey = val;
    if (!isInitializing)
      await setValue(SharePreferencesKey.BM_SECRET_KEY, '$val');
  }
