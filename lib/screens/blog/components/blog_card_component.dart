    ...
    return InkWell(
      onTap: () {
        if (widget.data.is_restricted.validate()) {
          ...
          BlogDetailScreen(blogId: widget.data.id.validate(), data: widget.data)
              .launch(context);
        }
      },
          ...
          children: [
            if (widget.data.embedded != null)
              RichText(
                text: TextSpan(
                  children: [
                    if (widget.data.embedded!.author!.first.name
                        .validate()
                        .isNotEmpty) ...[
                      TextSpan(
                        text:
                            widget.data.embedded!.author!.first.name.validate(),
                        style: secondaryTextStyle(
                            size: 14, fontFamily: fontFamily),
                      ),
                      if (widget.data.embedded!.author!.first.is_user_verified
                          .validate())
                        WidgetSpan(
                    ...
                    TextSpan(
                        text: '${convertToAgo(widget.data.date.validate())}  |',
                        style: secondaryTextStyle(
                            size: 14, fontFamily: fontFamily)),
                    TextSpan(
                        text: '  $category',
            ...
            16.height,
            Text(parseHtmlString(widget.data.title!.rendered.validate()),
                style: boldTextStyle()),
            16.height,
            Text(parseHtmlString(widget.data.excerpt!.rendered.validate()),
                style: secondaryTextStyle(size: 16)),
            if (widget.data.embedded != null &&
                widget.data.embedded!.featuredMedia != null)
              cachedImage(widget.data.embedded!.featuredMedia!.first.source_url
                      .validate())
