...
import 'package:socialNetwork/models/common_models/content.dart';
...
class _PostContentComponentState extends State<PostContentComponent> {
    ...
    if (widget.blogId != null) {
      ...
      await wpPostById(postId: widget.blogId.validate()).then((value) {
        ...
        if (value.is_restricted.validate()) {
          ...
          BlogDetailScreen(blogId: widget.blogId.validate(), data: value)
              .launch(context);
        }
      }).catchError((e) {
  ...
  Widget build(BuildContext context) {
    if (widget.contentObject.validate().isNotEmpty &&
        widget.contentObject
            .validate()
            .any((element) => element.isLink.validate()) &&
        !widget.postContent.validate().contains('<blockquote')) {
      return Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        mainAxisSize: MainAxisSize.min,
        children: widget.contentObject.validate().map((e) {
          if (e.isLink.validate()) {
            return AnyLinkPreview(link: parseHtmlString(e.content.validate()))
                .paddingSymmetric(vertical: 4);
          } else if (e.content.validate().validate().contains('is_mention=1')) {
            return HtmlWidget(postContent: e.content.validateAndFilter());
          } else {
            return Text(e.content.validate().nonBreaking,
      ...
      if (widget.postType == PostActivityType.newBlogPost) {
        return InkWell(
          ...
          child: HtmlWidget(
              postContent: widget.postContent.validateAndFilter(),
